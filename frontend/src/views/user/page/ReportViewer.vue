  ...
  <div class="report-viewer">
    ...
    <div class="toolbar">
      ...
      <div class="toolbar-right">
        <div class="zoom-controls">
          <button @click="zoomOut"><img src="/assets/icons/zoom_out.svg" alt="-" /></button>
          <span class="zoom-text">{{ zoomLevels[zoomIndex] }}%</span>
          <button @click="zoomIn"><img src="/assets/icons/zoom_in.svg" alt="+" /></button>
        </div>
        <button class="btn-print" @click="handlePrint">인쇄</button>
        <button class="btn-download" @click="handleDownload">리포트 다운로드</button>
      </div>
    </div>
    ...
    <div class="pdf-hidden">
      ...
      <div class="pdf-page cover-page" ref="page1">
        ...
        <h1 class="cover-title">쑥쑥 성장 로드맵 보고서</h1>
      ...
      <div class="pdf-page" ref="page2">
        ...
        <div class="page-top-info">
          ...
          <span class="file-id">{{ analysis?.patient_name }}_{{ formatDate(analysis?.birth_date, '') }}_a{{ analysis?.id }}</span>
          <span class="exam-date">검사일 {{ formatDate(analysis?.created_at) }}</span>
        </div>

        <!-- 환자 정보 테이블 -->
        <table class="patient-table">
          <tr>
            <th>성명</th><td>{{ analysis?.patient_name }}</td>
            <th>성별</th><td>{{ analysis?.gender === 'M' ? '남' : '여' }}</td>
            <th>생년월일</th><td>{{ formatDate(analysis?.birth_date) }}</td>
            <th>키</th><td>{{ analysis?.height }} <small>cm</small></td>
            <th>몸무게</th><td>{{ analysis?.weight }} <small>kg</small></td>
          </tr>
        </table>
        ...
        <div class="age-score-section">
          <div class="age-row">
            <span class="age-item">만나이 <strong>{{ ageDisplay.years }}세 {{ String(ageDisplay.months).padStart(2, '0') }}개월</strong></span>
            <span class="age-arrow">◀</span>
            <span class="age-item">뼈 나이(AI) <strong>{{ analysis?.bone_age_years }}세 {{ String(analysis?.bone_age_months || 0).padStart(2, '0') }}개월</strong></span>
        ...
        <div class="gauge-section">
          ...
          <div class="gauge-row">
            ...
            <span class="gauge-value"><strong>{{ analysis?.height }}</strong> cm</span>
            <div class="gauge-bar-wrapper">
              <img class="dino-small" src="/assets/report/dino_marker_small.png" />
              <div class="gauge-bar">
                <div class="gauge-fill" :style="{ width: heightPercentile + '%' }"></div>
                <div class="gauge-marker" :style="{ left: heightPercentile + '%' }">
                  <span class="marker-value">{{ heightPercentile }}</span>
                  <img src="/assets/report/dino_marker_progress.png" />
                </div>
              </div>
              <img class="dino-tall" src="/assets/report/dino_marker_tall.png" />
            </div>
            <div class="gauge-range"><span>작음(0)</span><span>큼(100)</span></div>
          </div>

          <!-- 유전 기반 예측 키 -->
          <div class="gauge-row">
            <div class="gauge-label-box genetic">유전 기반<br/>예측 키</div>
            <span class="gauge-value"><strong>{{ resultData?.Genetic_Predicted_Height || '-' }}</strong> cm</span>
            <div class="gauge-bar-wrapper">
              <img class="dino-small" src="/assets/report/dino_marker_small.png" />
              <div class="gauge-bar">
                <div class="gauge-fill" :style="{ width: geneticPercentile + '%' }"></div>
                <div class="gauge-marker" :style="{ left: geneticPercentile + '%' }">
          ...
          <div class="gauge-row">
            ...
            <span class="gauge-value"><strong>{{ resultData?.Growth_Curve_Predicted_Height || '-' }}</strong> cm</span>
            <div class="gauge-bar-wrapper">
              <img class="dino-small" src="/assets/report/dino_marker_small.png" />
              <div class="gauge-bar">
                <div class="gauge-fill" :style="{ width: boneAgePercentile + '%' }"></div>
                <div class="gauge-marker" :style="{ left: boneAgePercentile + '%' }">
          ...
          <div class="gauge-row">
            ...
            <span class="gauge-value"><strong>{{ resultData?.Final_Predicted_Height || '-' }}</strong> cm</span>
            <div class="gauge-bar-wrapper">
              <img class="dino-small" src="/assets/report/dino_marker_small.png" />
              <div class="gauge-bar">
                <div class="gauge-fill" :style="{ width: finalPercentile + '%' }"></div>
                <div class="gauge-marker" :style="{ left: finalPercentile + '%' }">
      ...
      <div class="pdf-page" ref="page3">
        ...
        <!-- 체질량 지수 -->
        <div class="health-gauge">
          <div class="health-title">체질량 지수 <strong>{{ bmiValue }} kg/m²</strong></div>
          <div class="level-bar level-4">
        ...
        <div class="health-gauge">
          ...
          <div class="level-bar level-5">
            ...
            <span :class="{ active: obesityLevel === '경도비만' }">경도비만</span>
            <span :class="{ active: obesityLevel === '중등도비만' }">중등도비만</span>
            <span :class="{ active: obesityLevel === '고도비만' }">고도비만</span>
          </div>
        ...
        <div class="chart-section">
          ...
          <div class="chart-with-desc">
            <canvas ref="chartCanvas3" width="560" height="380"></canvas>
      ...
      <div class="pdf-page" ref="page4">
        ...
        <div class="chart-section half">
          ...
          <p class="percentile-text" v-if="resultData?.Genetic_Predicted_Height">
            백분위 <strong>{{ geneticPercentile }}</strong> : 큰 순서로 상위 <strong>{{ 100 - geneticPercentile }}%</strong>에 해당하는 키 입니다.
          </p>
          <p class="percentile-text" v-else>백분위 <strong>-</strong> : 큰 순서로 상위 <strong>-%</strong>에 해당하는 키 입니다.</p>
          <div class="chart-with-desc">
            <canvas ref="chartCanvas4a" width="560" height="300"></canvas>
            <div class="chart-desc-box">
        ...
        <div class="chart-section half">
          ...
          <p class="percentile-text">
            백분위 <strong>{{ boneAgePercentile }}</strong> : 큰 순서로 상위 <strong>{{ 100 - boneAgePercentile }}%</strong>에 해당하는 키 입니다.
          </p>
          <div class="chart-with-desc">
            <canvas ref="chartCanvas4b" width="560" height="300"></canvas>
            <div class="chart-desc-box">
      ...
      <div class="pdf-page" ref="page5">
        ...
        <div class="chart-section">
          ...
          <p class="percentile-text">
            백분위 <strong>{{ finalPercentile }}</strong> : 큰 순서로 상위 <strong>{{ 100 - finalPercentile }}%</strong>에 해당하는 키 입니다.
          </p>
          <div class="chart-with-desc">
            <canvas ref="chartCanvas5" width="560" height="380"></canvas>
            <div class="chart-desc-box">
...
const loadAnalysis = async () => {
  ...
  resultData.value = typeof analysis.value.result_json === 'string'
...
const ageDiffText = computed(() => {
  ...
  const chronoM = (analysis.value.chronological_age_years || 0) * 12 + (analysis.value.chronological_age_months || 0)
  const boneM = (analysis.value.bone_age_years || 0) * 12 + (analysis.value.bone_age_months || 0)
...
const getPredictedPercentile = (predictedHeight, gender) => {
  ...
  return getPercentile(parseFloat(predictedHeight), growthHeightData, 216, gender)
}

const heightPercentile = computed(() =>
  getPercentile(parseFloat(analysis.value?.height), growthHeightData, ageMonths.value, analysis.value?.gender)
)
const geneticPercentile = computed(() =>
  getPredictedPercentile(resultData.value?.Genetic_Predicted_Height, analysis.value?.gender)
)
const boneAgePercentile = computed(() =>
  getPredictedPercentile(resultData.value?.Growth_Curve_Predicted_Height, analysis.value?.gender)
)
const finalPercentile = computed(() =>
  getPredictedPercentile(resultData.value?.Final_Predicted_Height, analysis.value?.gender)
)
...
const weightLevel = computed(() => {
  ...
  const wData = analysis.value.gender === 'M' ? growthWeightData.male : growthWeightData.female
  const row = wData.reduce((prev, curr) =>
    Math.abs(curr.month - ageMonths.value) < Math.abs(prev.month - ageMonths.value) ? curr : prev
  )
...
const bmiPercentile = computed(() =>
  getPercentile(parseFloat(bmiValue.value), growthBmiData, ageMonths.value, analysis.value?.gender)
)
...
const drawGrowthChart = (canvas, options) => {
  ...
  ctx.textAlign = 'center'; ctx.fillText('신장(cm)', 0, 0)
  ctx.restore()
...
const getCurrentHeightPredicted18 = () => {
  ...
  const gData = gender === 'M' ? growthHeightData.male : growthHeightData.female
  ...
  for (let i = 0; i < pList.length - 1; i++) {
    if (p >= pList[i].val && p <= pList[i + 1].val) {
      ...
      return row18[pList[i].key] + ratio * (row18[pList[i + 1].key] - row18[pList[i].key])
    }
  }
  return row18.p50
}
...
const generatePDF = async () => {
  ...
  const pages = [page1.value, page2.value, page3.value, page4.value, page5.value]
  ...
  for (let i = 6; i <= 15; i++) {
    pdf.addPage()
    const img = await loadImage(`/assets/report/common_${String(i).padStart(2, '0')}.jpg`)
...
onMounted(async () => {
  ...
  drawGrowthChart(chartCanvas3.value, {
    ...
    predictedAge: 216, predictedHeight: predicted18,
    xLabel: '만 나이(세)'
  })
  ...
  drawGrowthChart(chartCanvas4a.value, {
    ...
    predictedHeight: resultData.value?.Genetic_Predicted_Height ? parseFloat(resultData.value.Genetic_Predicted_Height) : null,
    xLabel: '만 나이(세)'
  })

  // 4p 하단: 뼈나이 기반 (X축 = 뼈나이)
  const boneAgeM = (analysis.value.bone_age_years || 0) * 12 + (analysis.value.bone_age_months || 0)
  drawGrowthChart(chartCanvas4b.value, {
    genderData: gData, currentAge: boneAgeM, currentHeight: curH,
    predictedAge: 216, predictedHeight: parseFloat(resultData.value?.Growth_Curve_Predicted_Height),
    xLabel: '뼈 나이(세)'
  })
  ...
  drawGrowthChart(chartCanvas5.value, {
    genderData: gData, currentAge: curAgeM, currentHeight: curH,
    predictedAge: 216, predictedHeight: parseFloat(resultData.value?.Final_Predicted_Height),
    xLabel: '만 나이(세)'
  })
...
.toolbar {
  ...
  .btn-back {
    ...
    img { width: 16px; filter: brightness(0) invert(1); }
  }
  ...
  .btn-print, .btn-download {
    ...
    background: none; color: #fff; cursor: pointer; font-size: 13px;
...
.loading { color: #fff; font-size: 18px; align-self: center; }
.pdf-iframe {
  width: 800px; height: 100%; border: none;
...
.page-top-info {
  ...
  .hospital { font-weight: bold; color: #333; }
}
...
.cover-page {
  ...
  padding: 60px 40px;
  .cover-logo { width: 180px; margin-top: 30px; }
  .cover-title { font-size: 32px; font-weight: 800; color: #333; margin: 25px 0; }
  .cover-info-box {
    ...
    .info-row {
      ...
      .label { font-weight: 700; min-width: 80px; color: #4A90D9; }
      .value { color: #333; }
    }
  }
  .cover-bg {
  ...
  .cover-diwave {
    width: 140px; position: absolute; bottom: 50px;
  }
}

// ===== 2페이지 =====
.patient-table {
  width: calc(100% - 80px); margin: 10px 40px; border-collapse: collapse; font-size: 13px;
...
.age-score-section {
  padding: 12px 40px;
  .age-row {
    display: flex; align-items: center; gap: 12px; font-size: 14px; margin-bottom: 8px;
    .age-arrow { color: #4A90D9; font-size: 18px; }
    .age-diff { color: #888; font-size: 12px; }
    strong { font-size: 18px; }
  }
  .score-row {
    display: flex; gap: 40px; font-size: 14px;
    strong { font-size: 20px; color: #4A90D9; }
  }
}
...
.gauge-section {
  padding: 0 40px;
  .section-title { font-size: 16px; font-weight: 700; margin: 15px 0 10px; }
}

.gauge-row {
  display: flex; align-items: center; gap: 10px; margin-bottom: 8px;
  .gauge-label-box {
    width: 80px; padding: 6px 8px; border-radius: 20px; text-align: center;
    font-size: 11px; font-weight: 700; color: #fff; line-height: 1.3;
    &.current { background: #4A90D9; }
    &.genetic { background: #5BC0BE; }
    &.bone { background: #F4A261; }
    &.final { background: #E76F51; }
  }
...
.gauge-bar {
  ...
  background: linear-gradient(to right, #2B5876, #4ECDC4, #e0e0e0);
  .gauge-fill {
  ...
  .gauge-marker {
    ...
    display: flex; flex-direction: column; align-items: center;
    .marker-value { font-size: 12px; font-weight: 700; }
    img { width: 28px; }
  }
}

.gauge-range {
  display: flex; justify-content: space-between; padding: 0 44px 0 44px;
  font-size: 10px; color: #888; margin-top: -2px;
}

.correction-row {
  display: flex; justify-content: center; gap: 30px; margin: 6px 0;
  .correction {
    padding: 4px 14px; border: 1px solid #ddd; border-radius: 20px;
...
// ===== 스코어 설명 =====
.score-desc {
  padding: 10px 40px; font-size: 10px; color: #666; line-height: 1.6;
...
.health-gauge {
  padding: 8px 40px;
  .health-title { font-size: 15px; margin-bottom: 6px;
    strong { font-size: 20px; }
  }
  .health-desc { font-size: 9px; color: #888; margin: 4px 0 0; line-height: 1.5; }
}
...
.level-bar {
  ...
  span {
    ...
    background: #f0f0f0; color: #999; border-right: 1px solid #ddd;
    &:last-child { border-right: none; }
    &.active { background: #4A90D9; color: #fff; font-weight: 700; }
  }
}
...
.chart-with-desc {
  ...
  .chart-desc-box {
    width: 160px; padding: 12px; background: #f8f9fa; border-radius: 6px;
    font-size: 11px; color: #666; line-height: 1.5;
    strong { display: block; color: #333; margin-bottom: 4px; }
  }
}
...
.chart-legend {
  ...
  .legend-dot {
    ...
    &.current { background: #4ECDC4; }
    &.predicted { background: #FF6B6B; }
  }
}
